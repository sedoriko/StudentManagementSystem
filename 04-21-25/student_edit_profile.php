...
// Fetch password from `users` table for verification
$sql_user = "SELECT password FROM users WHERE user_id = ?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        // Update email in `students`
        $update_stmt = $conn->prepare("UPDATE students SET email = ? WHERE user_id = ?");
        $update_stmt->bind_param("si", $email, $user_id);
        $success_student = $update_stmt->execute();

        // Update password in `users` if changed
        if (!empty($new_password)) {
            $hashed_password = password_hash($new_password, PASSWORD_BCRYPT);
            $update_user_stmt = $conn->prepare("UPDATE users SET password = ? WHERE user_id = ?");
            $update_user_stmt->bind_param("si", $hashed_password, $user_id);
            $success_user = $update_user_stmt->execute();
        } else {
            $success_user = true;
        }

        if ($success_student && $success_user) {
